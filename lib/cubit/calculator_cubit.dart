...
class CalculatorCubit extends Cubit<CalculatorState> {
  ...
  void calculate() {
    ...
    final evaluable = _lastIsOperator
        ? state.tokens.sublist(0, state.tokens.length - 1)
        : state.tokens;
  ...
  void clear() =>
      emit(CalculatorState.initial().copyWith(memoryValue: state.memoryValue));
  ...
  double? _currentValue() {
    ...
    return double.tryParse(state.resultLine);
  }

  void memoryClear() {
    if (!state.hasMemory) return;
    emit(state.copyWith(memoryValue: 0));
  }

  void memoryAdd() {
    final value = _currentValue();
    if (value == null) return;
    emit(state.copyWith(memoryValue: state.memoryValue + value));
  }
  ...
  void memoryRecall() {
    if (!state.hasMemory) return;
    final memStr = CalculatorEngine.format(state.memoryValue);


    if (state.hasError || state.justEvaluated) {
      _emitTokens([memStr], justEvaluated: false);
      return;
    }
